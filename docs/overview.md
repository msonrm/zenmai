...
| 入力 | 前置詞 18 / 動詞 126 / 物 134 / 上位語 31 —— 日本語の言い方は延べ **1,097** |
| 打ち方 | キーボード / ゲームパッド / スマホのフリック（★**op の語彙が同じなので配線は 1 本**） |
| 検査 | **8 本**（`test/run-*.js`）—— 入力の固定表 65 件・通しの日本語・walkthrough の未訳ログ・Glk シム・セーブ復帰・ブラウザホストの配線・ZVM ハーネス・★**語の食い合い**（短い読みが語の中を食って別の物を指すのを見張る門。ルビ表 + カタカナ 455 語） |
...
- **英語モード**（本文もコマンドも英語）は未着手。土台はある（訳を引かず素通し・`toCommand` を
  ...
  `native/main.c` の `zork_sym()` と row 0（方向キーなし = 記号の面）にある。
