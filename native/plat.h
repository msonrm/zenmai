/* Zenmai プラットフォーム境界。
 *
 * ★**この 10 本だけが機械に触る。** 描画の芯（render.c）・組版・グリフ・履歴・窓・
...
/* ★★フェイスボタンの**物理位置**を教える。
 *
 * ★SDL の A/B/X/Y は**機体に印刷された札の名前**であって位置ではない —— PortMaster の
 *   定義は札に合わせるので、同じ `a` が任天堂式の機体では右、Xbox 式では下に来る。
 *   ★**SDL からは位置を復元できない**ので、初回起動で本人に押してもらって決める。
 *
 * 引数は**素の対応表での BTN_***（＝一度もこれを呼んでいない状態の並び）で、
 * 右／下／上／左 の各位置に来るものを渡す。★絶対指定なので何度呼んでも積み重ならない。
 * 既定に戻すには pad_face_order(BTN_CIR, BTN_X, BTN_TRI, BTN_SQ)。
 *
 * ★PS1 では何もしない —— PlayStation のパッドは △○✕□ の位置が決まっている。 */
