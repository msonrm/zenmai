...
#
# 収録する字は **KH ドットフォントの cmap（7,623 字）と同じ集合**にしてある。
...
"$PY" - "$NOTO" "$KH/KH-Dot-Hibiya-24.ttf" "$TMP" <<'PYEOF'
import sys
from fontTools.ttLib import TTCollection, TTFont
noto, kh, tmp = sys.argv[1], sys.argv[2], sys.argv[3]
...
codes = sorted(TTFont(kh).getBestCmap().keys())
open(f"{tmp}/set.txt", "w").write(",".join(f"U+{c:04X}" for c in codes))
print(f"収録する字: {len(codes):,}（KH ドットフォントの cmap と同一）")
PYEOF
