...
/* ★**行の高さ（器）**。24 が既定で、PS1 はこれしか採れない ——
 *   焼いたビットマップは 24 行に収まる字形で作ってあり、2MB RAM に余分な行は払えない。
 *
 ...
 *   ★**席ごとに大きさを変えるのではなく、全席で器を広げる** —— `glyph_ft.c` の
 *   `fit_px` の注記どおり「送り幅と描く大きさは常に一致していなければならない」ので、
