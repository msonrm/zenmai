...
# ★★SDL 版だけが持つ「ボタンの並び」を**あらかじめ置いておく**。
#   置かないと初回起動の訊く画面が出て、PS1 版と起動の流れがずれる ——
#   ★この検査は 2 つのビルドが**同じ画面**を出すかを見るものなので、
#   片方にしかない画面はここでは邪魔になる（訊く画面自体は test-face.sh が見る）。
#   ★毎回書き直す。残っていたものを使うと「古い成果物」の罠になる。
...
# ★★以前はここで**オプションのメニュー**を比べていたが、比べられなくなった ——
#   「ボタン設定」は SDL 版にしかない（PS1 のパッドは位置が決まっている）ので、
#   ★**項目の数がビルドで違うのは設計どおり**。設計どおりの差を検査で追うと、
#   「赤いのが正しい」状態を作ってしまう。
# ★代わりに**全画面の図**を比べる。帯を積んで送る仕組み（gp0_upload）は板と同じで、
#   しかも★**ボタンの位置を触ったいま、いちばん壊れやすいのはこの図**なので、
#   置き換えでむしろ守りが厚くなっている。
# ★SDL のメニューの並び自体は test-face.sh が見る（↓↓ で「ボタン設定」に届くか）。
run "ひらがな入力方法の図（帯を積んで送る）" help_type.script 900
