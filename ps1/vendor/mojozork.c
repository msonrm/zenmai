        ...
        *(GState->sp++) = numlocals;  // number of locals we're allocating.
        ...
        if (GState->header.version <= 4) {
            for (i = 0; i < numlocals; i++, routine += sizeof (uint16)) {
                // ZENMAI PATCH: routine は奇数アドレスになりうる。非アラインの u16 ロードは
                // MIPS(PS1)で例外/誤読になるため memcpy(生バイトコピー・意味論同一)にする。
                uint16 rawlocal;
                memcpy(&rawlocal, routine, sizeof (uint16));  // leave it byteswapped when moving to the stack.
                *(GState->sp++) = rawlocal;
            }
        } else {
...
static void opcode_loadw(void)
{
    ...
    // ZENMAI PATCH: offset は奇数になりうる(非アライン)。memcpy で生コピー(byteswap なし)。
    memcpy(store, src, sizeof (uint16));  // copy from bigendian to bigendian: no byteswap.
}
...
static char decode_zscii_char(const uint16 val)
{
    ...
    if ((val >= 32) && (val <= 126)) {
        ...
        ch = '\n';
    } else if (val == 9 || val == 11) {  /* ZENMAI PATCH: tab/sentence space → 空白(ZVM と同じ) */
        ch = ' ';
    } else if (val == 0) {
...
{
    char buf[2048];  /* ZENMAI PATCH: Zork I の最長文字列(約 1.3KB)を malloc なしで収める */
    char *ptr = buf;
...
static void inititialOpcodeTableSetup(void)
{
    ...
    if (GState->header.version < 3) {  // most Infocom games are version 3.
        return;  // we're done.
    }
